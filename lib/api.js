...
// Function to simulate a model response
// Update this function in lib/api.js to simplify the simulation responses
...
export async function simulateModelResponse({
  ...
  // Generate a basic simulated response based on position
  let response = '';
  
  if (position === 'pro') {
    response = `I think ${productName} is a great opportunity. [Position: Pro, Round: ${round}]`;
  } else if (position === 'against') {
    response = `I have concerns about ${productName}. [Position: Against, Round: ${round}]`;
  } else { // neutral
    response = `${productName} has both potential and challenges. [Position: Neutral, Round: ${round}]`;
  }
  
  // Add eliminated member context if applicable
  if (eliminatedMembers.length > 0 && round > 1) {
    response += ` After ${eliminatedMembers.join(", ")} was eliminated, I see this is getting competitive.`;
  }
  
  // Add model signature
  response += ` [${modelShortName}]`;
